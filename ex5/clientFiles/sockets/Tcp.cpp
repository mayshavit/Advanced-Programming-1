...
* The Input: string data to send, int client.						   *
* The output: int number representing the return status		           *
* The Function operation: sending the required data, using his length, *
* the socket descriptor, and the client number.						   *
***********************************************************************/
int Tcp::sendData(string data, int client) {
	ssize_t data_len = data.length();
	const char * datas = data.c_str();
	ssize_t sent_bytes = send(this->isServer ? client:
...
***********************************************************************/
int Tcp::reciveData(char* buffer, int size, int client) {
	ssize_t read_bytes = recv(this->isServer ? client:
...
/***********************************************************************
     * function name: connectAclient
     * The input : none
     * The output : int number, representing an "id" for client. Each client
     * has its own id.
     * The Function operaton:
     */
int Tcp::connectAClient() {
	int client = 0;
	struct sockaddr_in client_sin;

	unsigned int len_of_add = sizeof(client_sin);
	client = accept(this->socketDescriptor, (
	struct sockaddr *) &client_sin, &len_of_add);

	if (client < 0){
		//if there was a problem accepting a client
		perror("ERROR_CONNECT\n");
		exit(1);
	}
